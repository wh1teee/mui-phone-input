...
- The canonical npm name is `@wh1teee/mui-phone-input`, matching the authenticated npm and GitHub identity verified on 2026-08-04. The first registry publication, create/publish authority, and 2FA/Trusted Publishing proof remain tracked by the human release gate `mpi-g7a`, which blocks registry canary publication but not workspace or donor implementation.
