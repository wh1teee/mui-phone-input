...
2. Deprecate the affected RC with an actionable replacement message:
   ...
   ```bash
   npm deprecate @wh1teee/mui-phone-input@0.1.0-next.6 "RC withdrawn; use 0.1.0-next.5 until a replacement RC is published."
   ```

3. Restore the `next` dist-tag to the last verified prerelease without changing `latest`:

   ```bash
   npm dist-tag add @wh1teee/mui-phone-input@0.1.0-next.5 next
   ```

4. Mark the matching GitHub prerelease as withdrawn and link the replacement when one exists.
5. Do not rebuild, overwrite, or republish `0.1.0-next.6`. A correction must use a new immutable prerelease version and a new Git tag.
