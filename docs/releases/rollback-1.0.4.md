...
3. Mark the affected version without changing its bytes:
   ...
   ```sh
   npm deprecate @wh1teee/mui-phone-input@1.0.4 "Withdrawn; pin the last verified version while a corrective patch is prepared."
   ```

4. Restore the `latest` dist-tag only to a separately verified stable artifact.
   Do not point `latest` at an unpublished or failed tag. For example:

   ```sh
   npm dist-tag add @wh1teee/mui-phone-input@<verified-version> latest
   ```
